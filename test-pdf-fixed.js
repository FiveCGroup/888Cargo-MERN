...
function verifyData() {
    return new Promise((resolve, reject) => {
        db.all("SELECT * FROM carga WHERE codigo_carga = 'TEST123'", (err, cargas) => {
            if (err) {
                reject(err);
                return;
            }
            ...
            cargas.forEach(carga => {
                console.log(`- ID: ${carga.id_carga}, Código: ${carga.codigo_carga}, Destino: ${carga.ciudad_destino}`);
            });
            ...
            db.all(`
                ...
                WHERE carga.codigo_carga = 'TEST123'
            `, (err, qrs) => {
                if (err) {
                    reject(err);
                    return;
